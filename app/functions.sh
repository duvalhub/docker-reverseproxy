...
function docker_list_services {
    local service_state="$1"
    if [ -z "$service_state" ]; then
      log_error "docker_list_services needs a destination file as first parameter."
      return 1
    fi
    local command="/services"
    docker_api "$command" > "$service_state"
    log_debug "Service State looks like : $(cat "$service_state" | cut -c1-100)" >&2
}
...
function get_nginx_proxy_container {
    if [ -z "$NGINX_PROXY_LABEL" ]; then 
     log_error "You must set NGINX_PROXY_LABEL environment variable which is a label on the nginx proxy container"
     return 1
    fi
    local volumes_from
    # First try to get the nginx container ID from the container label.
    local nginx_cid="$(labeled_cid "$NGINX_PROXY_LABEL")"
...
function reload_nginx {
    ...
    if [[ -n "${_nginx_proxy_container:-}" ]]; then
        log_info "Reloading nginx proxy (${_nginx_proxy_container})..."
      ...
      log_error "Can't Reload Nginx. Fatal error"
      return 1
    fi
...
function set_ownership_and_permissions {
  ...
  log_debug "checking $path ownership and permissions."
  ...
  if [[ -e "$path" ]]; then
    if [[ "$(stat -c %u:%g "$path" )" != "$user_num:$group_num" ]]; then
      log_debug "setting $path ownership to $user:$group."
      if [[ -L "$path" ]]; then
    ...
    if [[ -d "$path" ]]; then
      if [[ "$(stat -c %a "$path")" != "$d_perms" ]]; then
        log_debug "setting $path permissions to $d_perms."
        chmod "$d_perms" "$path"
      fi
